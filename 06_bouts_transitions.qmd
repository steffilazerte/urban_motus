...
- this means we cannot simply compare runs on sequential rows in a data frame (using `lag()` or `lead()`, for example)
...
collected in [Basic Filtering](04_basic_filters.html) and [Fine-scale Filtering](05_fine_filters.html).
We will finish off by omitting any run with a length of 2 or less (this is a partial application of the 
...
In this step we'll be identifying unlikely movements between stations (i.e. super fast birds, and those going the wrong way).
...
We will assume the following limits for `max_flight_speed` and `max_tower_dist` (defined in [Setup](XX_setup.html#define-constants)).

- Flight speed `r max_flight_speed` m/s (`max_flight_speed`)
...
- Tower distance `r max_tower_dist` km (`max_tower_dist`)
...
- Here we use the units package to handle the unit conversions
- Units can then be compared with the `max_flight_speed` in units of `m/s`
- We defined a `min_time` which is the minimum amount of time it can take to
  transition between two stations acording to their distance the the `max_flight_speed`.
...
dist <- st_distance(recvs_sf) |>
  as_tibble() |>
  mutate(recv1 = recvs_sf$recvDeployID) |>
  relocate(recv1) |>
  rename_with(\(x) as.character(recvs_sf$recvDeployID), .cols = -1) |>
  pivot_longer(-recv1, names_to = "recv2", values_to = "next_dist") |>
  mutate(recv2 = as.integer(recv2)) |>
  arrange(recv1, recv2) |>
...
write_rds(dist, "Data/02_Datasets/stn_dist.rds")
```
...
group is within `r max_tower_dist` km of the others. 
...
groups <- nest(runs, .by = c("tagDeployID")) |>
  mutate(data = future_map(data, \(x) {
    ...
    g <- dist |>
      filter(recv1 %in% unique(x$recvDeployID), 
             recv2 %in% unique(x$recvDeployID)) |>
      filter(recv1 != recv2) |>
      summarize(
        stn_grouped_with = list(recv2[next_dist < max_tower_dist]), 
        stn_group = paste0(sort(unique(c(recv1, recv2[next_dist < max_tower_dist]))), 
                           collapse = "-"),
        .by = "recv1") |>
      # Only keep groups which are common across stations
      mutate(stn_group = if_else(n() > 1, stn_group[1], as.character(recv1)[1]), 
             .by = "stn_group")
    
    left_join(x, g, by = c("recvDeployID" = "recv1")) |>
...
the course of 10 runs (all within `r bout_cutoff` minutes of each other),
we would collapse this down into one detection bout of 6 hours. 

We'll use a `bout_cutoff` of `r bout_cutoff` minutes defined in [Setup](XX_setup.html#define-constants).
So if two runs at the same station are more than `r bout_cutoff` min apart, we'll consider
...
bouts <- nest(groups, .by = c("tagDeployID", "stn_group")) |>
  ...
  mutate(dateBegin = as_date(timeBegin),
         ...
         id = paste0(tagDeployID, "_", stn_group, "_", bout_id)) |>
...
```{r}
write_rds(trans, "Data/02_Datasets/transitions_initial.rds")
write_rds(bouts, "Data/02_Datasets/bouts_initial.rds")
```
