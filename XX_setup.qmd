...
We only need to do this to one database, as the `metadata()` data step in [Download/Update data](02_download.html) adds full species lists to all databases (so metadata is the same
...
## Functions
These are the custom functions used in this workflow.
Some functions are displayed in the various workflow pages, but they are all
sourced from `XX_functions.R` (as is the following).
