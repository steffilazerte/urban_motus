...
processing of this data into feather `hits` and `runs` (See [Save to Arrow/Feather](#save-to-arrowfeather)).
...
Now save these as Arrow/Feather format (but first remove any previously saved data, 
as we'll be saving to Arrow by splitting the data across multiple files, 
so don't want any conflicts).
...
The thing with splitting data across multiple files is that we ideally want files
to be >20MB and <2GB and we want to avoid too many files (< 10,000).
...
Not fantastic, but we want the data to be split in a sensible way, so we'll leave
it as is.
