...
write_csv(stn_time, "Data/03_Final/summary_daily_bouts.csv")
```


## Overall Summary

Summarize the overall patterns and number of samples in the data.
...
2. Being detected around a single receiver (or receiver group) on and off
   within a week.
...
filter(b0, tagDeployID == 41209, dateBegin < "2022-12-01") |>
  ggplot(aes(x = start, xend = end, y = dateBegin, colour = stn_group)) +
  geom_segment(linewidth = 1.5) +
  labs(x = "Hour (Local timezone)", title = "tagDeployID 41224 (Spotted Towhee)")
```
