...
trans <- read_rds("Data/02_Datasets/transitions_initial.rds")
bouts <- read_rds("Data/02_Datasets/bouts_initial.rds")
dist <- read_rds("Data/02_Datasets/stn_dist.rds")
```
...
1. **Transitions which are too fast** - We have a cutoff of `r max_flight_speed` `r units(max_flight_speed) |> as.character()`
    ...
    - These can be identified *and* fixed automatically (but will still be manually reviewed, just in case)

2. **Transitions which are in the wrong direction**
    - It's highly unlikely that a migrating bird will start migration and then 
      backtrack hundreds of kilometres. So 'false' starts, likely false positives some where in those transitions.
    - These can be identified automatically, but are prone to being overzealous so 
      need to be assessed manually. It can also be tricky to identify which bout is the problem.
      
3. **Transitions which include a suspiciously short bout - NOT IMPLEMENTED**
...
:::{.callout-tip}
#### Note
Even programatically fixed problems will be manually reviewed in the [Manual Checks](#manual-checks) section below.
:::
...
```{r}
write_rds(all_bad, "Data/02_Datasets/bad_bouts.rds")
```
